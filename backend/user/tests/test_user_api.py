...
TOP_UP_URL = reverse('user:topup_balance')
...
class PrivateUserApiTests(TestCase):
    ...
    def test_update_restricted_fields_returns_error(self):
        ...
        payload = {'balance': Decimal('10000'), 'role': 'AD'}
        ...
        self.assertEqual(self.user.get_role_display(), role)

    def test_topping_up_success(self):
        """Test topping up the authenticated user's balance."""
        payload = {'amount': '1000'}
        new_balance = self.user.balance + Decimal(payload['amount'])

        res = self.client.post(TOP_UP_URL, payload)
